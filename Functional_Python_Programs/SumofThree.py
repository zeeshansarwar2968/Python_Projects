...
def find_triplets(n, arr):
    ...
    for i in range(0, n - 2):  # Looping in to the array elements till (n-2) element for the 'i' th element
        for j in range(i + 1, n - 1):  # Looping in to the array from the (i+1)th elements position till last element
            for k in range(j + 1, n):  # Taking the jth elements position till the last position
